...
// vetor que irá armazenar um objeto aposta (com nº do cavalo e valor da aposta)
const apostas = [];
...
frm.addEventListener("submit", (e) => {
    ...
    apostas.push({cavalo, valor}); // equivalente a: apostas.push({ cavalo: cavalo, valor: valor });
...
frm.inCavalo.addEventListener("blur", () => {
    ...
    // (não exibe mensagem de alerta, pois pode sair com um clique em Ganhador)
    ...
    // exibe nome, nº de apostas e total apostado do cavalo
    respCavalo.innerText = `${nome} (Apostas: ${contaNum} - R$: ${total.toFixed(2)})`;
});



const obterCavalo = (num) => { // função para obter o cavalo a ser apostado
...
frm.btGanhador.addEventListener("click", () => {
    // solicita o número do cavalo ganhador (já converte o para número)
    ...
    resumo += `Total Geral R$: ${totalizarApostas(ganhador).toFixed(2)}`;
